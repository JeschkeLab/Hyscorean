...
if FitData.GUI
  ...
  FrequencyAxis = linspace(-1/(2*Exp.dt),1/(2*Exp.dt),length(dispData));
  warning('off','all')
  grid(hAx,'on')
  hold(hAx,'on')
  ...
  set(hAx,'XLim',[-SimOpt.FreqLim SimOpt.FreqLim]);
  set(hAx,'YLim',[0 SimOpt.FreqLim]);
  set(hsubAx1,'XLim',[-SimOpt.FreqLim SimOpt.FreqLim]);
  set(hsubAx2,'XLim',[-SimOpt.FreqLim 0]);
...
if FitData.GUI
  ...
  set(findobj('Tag','currsimdata'),'ZData',NaN*ones(length(FitData.ExpSpec),length(FitData.ExpSpec)));
  set(findobj('Tag','currsimdata_projection2'),'YData',NaN*ones(1,length(FitData.ExpSpec)));
  set(findobj('Tag','currsimdata_projection1'),'YData',NaN*ones(1,length(FitData.ExpSpec)));
...
if numel(SimSystems)==1
  [t1,t2,~,out{1:FitData.nOutArguments}] = FitData.SimFcn(SimSystems,Exp,SimOpt);
else
  [t1,t2,~,out{1:FitData.nOutArguments}] = FitData.SimFcn(SimSystems,Exp,SimOpt);
end
...
if SimOpt.Lorentz2GaussCheck
  Processed.TimeAxis1 = t1;
  Processed.TimeAxis2 = t2;
  Processed.Signal = tdx;
  [Processed]=Lorentz2Gauss2D(Processed,SimOpt.L2GParameters);
  tdx = Processed.Signal;
end
%Use same apodization window as experimental data
tdx = apodizationWin(tdx,SimOpt.WindowType,SimOpt.WindowDecay);
%Fourier transform with same zerofilling as experimental data
simspec = fftshift(fft2(tdx,SimOpt.ZeroFillFactor*Exp.nPoints,SimOpt.ZeroFillFactor*Exp.nPoints));
...
if FitData.GUI && (UserCommand~=99) 
    FrequencyAxis = linspace(-1/(2*Exp.dt),1/(2*Exp.dt),length(ExpSpec));
    
    %not sure about this...
    simspec = simspec - simspec(end,end);
    FitData.bestspec = FitData.bestspec - FitData.bestspec(end,end);

  % update contour graph
  set(findobj('Tag','expdata'),'XData',FrequencyAxis,'YData',FrequencyAxis,'ZData',ExpSpec);
  set(findobj('Tag','bestsimdata'),'XData',FrequencyAxis,'YData',FrequencyAxis,'ZData',abs(FitData.bestspec)/max(max(abs(FitData.bestspec))));
  set(findobj('Tag','currsimdata'),'XData',FrequencyAxis,'YData',FrequencyAxis,'ZData',abs(simspec)/max(max(abs(simspec))));
  % update upper projection graph
  Inset = sum(ExpSpec(:,round(length(ExpSpec)/2,0):end),2);
  set(findobj('Tag','expdata_projection2'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  Temp = abs(FitData.bestspec)/max(max(abs(FitData.bestspec)));
  Inset = sum(Temp(:,round(length(Temp)/2,0):end),2);
  set(findobj('Tag','bestsimdata_projection2'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  Temp = abs(simspec)/max(max(abs(simspec)));
  Inset = sum(Temp(:,round(length(Temp)/2,0):end),2);
  set(findobj('Tag','currsimdata_projection2'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  % update lower projection graph  
  Inset = sum(ExpSpec(round(length(ExpSpec)/2,0):end,:));
  set(findobj('Tag','expdata_projection1'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  Temp = abs(FitData.bestspec)/max(max(abs(FitData.bestspec)));
  Inset = sum(Temp(:,round(length(Temp)/2,0):end),2);
  set(findobj('Tag','bestsimdata_projection1'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  Temp = abs(simspec)/max(max(abs(simspec)));
  Inset = sum(Temp(:,round(length(Temp)/2,0):end),2);
  set(findobj('Tag','currsimdata_projection1'),'XData',FrequencyAxis,'YData',Inset/max(Inset));
  
  if strcmp(FitOpts.Scaling, 'none')
    dispData = [FitData.ExpSpec;real(FitData.bestspec).';abs(simspec).'];
    maxy = max(max(dispData)); miny = min(min(dispData));
    YLimits = [miny maxy] + [-1 1]*FitOpt.PlotStretchFactor*(maxy-miny);
