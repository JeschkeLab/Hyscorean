function  contour2lineplot(ContourHandle,ReductionStep)
...
for ContourIndex=1:numel(Data)
   ...
   RGB_colors = vals2colormap(Data(ContourIndex).level, 'parula', [Data(1).level Data(end).level ]);
  
   %Plot the current contour line
   plot(Data(ContourIndex).xdata,Data(ContourIndex).ydata,'color',RGB_colors)
