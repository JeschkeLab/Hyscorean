function [Data]=mountData(RawData,options)
% Mount loaded data into appropiate structures for further processing.
% -----------INPUT------------------------------------------------
...
%
% Original script from S. Pribitzer
% Adapted for TrierAnalysis2018 by Luis Fabregas
...
switch options.type
  case 'experimental'
    ...
    if isfield(Measurement.trier,'NUS')
          ...
          [Data]=mountNUSdata(RawData,options);
          return
    else
      Data.NUSflag = false;
    end
    ...
    % Load the file to get timings and set up t_2 axis and zero time
    tauAbsolute1 =  2*PulseSequence{2}.t;   % absolute time of first echo
    tauRelative1 = PulseSequence{2}.t;     % relative (to the preceeding pulse) time of first echo
    tauRelative2 = PulseSequence{4}.t - PulseSequence{2}.t - tauRelative1;  % relative (to the preceeding pulse) time of the second echo
    tauAbsolute2 = PulseSequence{4}.t + tauRelative2; % absolute time of second echo
    
    TimeAxis2 = -(Measurement.trier.parvars{2}.axis - tauAbsolute2); % adjust t_2 axis to zero time
    TimeAxis1(1) = PulseSequence{3}.t - tauAbsolute1;  % first element of t1 vector from file name
...
%       colormap jet
  ...
  case 'simulation'
    ...
    Data.PreProcessedSignal = RawData.z;
