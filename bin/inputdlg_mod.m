...
for lp=1:NumQuest,
  ...
  EditHandle(lp)=uicontrol(InputFig    , ...
    EdInfo      , ...
    'Max'        ,NumLines(lp,1)       , ...
    'Position'   ,[2*TxtXOffset EditYOffset(lp) 150*TxtWidth EditHeight(lp)], ...
    'String'     ,DefAns{lp}           , ...
    'Tag'        ,'Edit'                 ...
    );
  ...
  if (size(NumLines,2) == 2)
    ...
    EditWidth = setcolumnwidth(EditHandle(lp), NumLines(lp,1), 100*NumLines(lp,2));
    EditWidth = 523;
...
if ~inputWidthSpecified
  TxtWidth=FigWidth-2*DefOffset;
  for lp=1:NumQuest
    set(EditHandle(lp), 'Position', [TxtXOffset EditYOffset(lp) 100*TxtWidth EditHeight(lp)]);
  end
end
