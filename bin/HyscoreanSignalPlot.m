 ...
 if ~isfield(handles,'PlotProcessedSignal')     
  ...
  if ~isfield(handles,'PlotBackground')     
   PlotBackground = false;
    ...
    PlotBackground = get(handles.PlotBackground,'Value');
  end
...
%     trace1 = trace1/max(abs(trace1));
  ...
  if get(handles.PlotApodizationWindow,'value')
    ...
    if PlotBackground
      ...
      text(1.1*Axis(handles.Data.BackgroundStartIndex1),ylimMax,sprintf('%i ns',round(1000*Axis(handles.Data.BackgroundStartIndex1),0)),'Color',[0.6 0.0 0.8])
...
set(handles.signal_t1,'FontSize',8)
  ...
  if get(handles.NonCorrectedTrace,'value')
    ...
    trace2 = trace2/max(max(real(handles.Data.NonCorrectedIntegral)));
